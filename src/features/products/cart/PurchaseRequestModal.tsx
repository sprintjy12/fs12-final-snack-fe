...
/**
 * 장바구니 → 구매 요청 확인 모달.
 * 행별 "즉시 요청"(품목 1개)과 사이드바 "구매 요청"(선택 품목 전체) 둘 다
 * 이 모달을 재사용합니다. 요청 메시지 입력 후 제출하면 완료 화면으로 이어집니다.
 */
        ...
        >
            ...
            >
                <header className="flex items-center justify-between gap-3">
                    ...
                    <button
                        type="button"
                        className="grid size-10 place-items-center rounded-full border-0 bg-transparent text-foreground-muted"
                        aria-label="닫기"
                        onClick={onClose}
                    >
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="rounded-2xl border border-border bg-surface-muted px-4 py-4">
                        <ul className="m-0 flex list-none flex-col gap-4 p-0">
                            {items.map((item) => {
                                const photoSrc = getProductPhotoSrc(item.product.photo);
                ...
                <div className="flex gap-3">
                    <button
                        type="button"
                        className="flex h-14 flex-1 items-center justify-center rounded-2xl border-0 bg-snack-background-500 text-lg font-semibold text-accent"
                        onClick={onClose}
                    >
                        취소
                    </button>
