...
/**
 * Figma `kebab-menu` — 상품 수정 / 상품 삭제
 * 상세·등록 내역에서 공통 사용
 */
export function ProductOwnerMenu({
  productName,
  onEdit,
  onDelete,
  size = "md",
  ...
  const focusItem = (index: number) => {
    const next =
      ((index % MENU_ITEM_COUNT) + MENU_ITEM_COUNT) % MENU_ITEM_COUNT;
  ...
  const itemClassName = isMd
    ? "flex w-full cursor-pointer items-center justify-center bg-transparent px-[26px] py-3.5 text-lg leading-[26px] font-normal text-snack-black-200 hover:bg-snack-background-300 focus-visible:bg-snack-background-300 focus-visible:outline-none"
    : "flex w-full cursor-pointer items-center justify-center bg-transparent px-[18px] py-2.5 text-sm leading-6 font-normal text-snack-black-200 hover:bg-snack-background-300 focus-visible:bg-snack-background-300 focus-visible:outline-none";
